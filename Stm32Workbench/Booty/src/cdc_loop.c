...
static const struct usb_string_descriptor manuf_desc_en = USB_STRING_DESC("OPP");
static const struct usb_string_descriptor prod_desc_en  = USB_STRING_DESC("Boot0.0");
...
static void cdc_loopback(usbd_device *dev, uint8_t event, uint8_t ep) {
	...
	// Grab rcv data
    rxLen = usbd_ep_read(dev, CDC_RXD_EP, &rxBuf[0], CDC_DATA_SZ);
    if (rxLen > 0)
    {
    	txLen = booty_rxtx_proc(&rxBuf[0], rxLen, &txData_p);
    	if (txLen)
    	{
			(void)usbd_ep_write(dev, CDC_TXD_EP, txData_p, txLen);
    	}
    }
}
...
int main(void)
{
	/* Note:  verify app only returns if the application is bad or if
	 *   PC13 is held low during boot (forcing board into the bootloader)
	 */
	booty_verify_app();
